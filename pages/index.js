...
function App() {
  ...
  return (
    <div className="whiteblue-bg">
      <div className="whiteblue-container">
        <h2 className="whiteblue-title">JRCM 走行解析アプリ</h2>
        <input
          type="file"
          accept=".csv"
          onChange={e => setFile(e.target.files[0])}
          className="whiteblue-input"
        /><br />
        <textarea
          value={instruction}
        ...
        ></textarea>
        <br />
        <button onClick={handleSend} disabled={loading} className="whiteblue-button">
          送信
        </button>
        {loading && <div className="whiteblue-loading">送信中...</div>}

        <div className="whiteblue-result">
          <strong>結果:</strong>
          <pre style={{ whiteSpace: 'pre-wrap' }}>
            {result ? result.replace(/---BEGIN VEGA---[\s\S]+---END VEGA---/, '') : ''}
          </pre>
        </div>
